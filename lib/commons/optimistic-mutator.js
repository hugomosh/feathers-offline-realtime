...
class Service {
  ...
  get(uuid, params) {
    ...
    return Promise.resolve(records[index]).then(select(params, ...this._alwaysSelect));
  }
  ...
  _create(data, params) {
    ...
    this._replicator._service.create(shallowClone(data), params).catch(() => {
      this._mutateStore('removed', data, 2);
    });

    return Promise.resolve(data).then(select(params, ...this._alwaysSelect));
  }
  ...
  _update(uuid, data, params) {
    ...
    // Start actual mutation on remote service
    this._replicator._service.update(getId(data), shallowClone(data), params).catch(() => {
  ...
  _patch(uuid, data, params) {
    ...
    // Start actual mutation on remote service
    this._replicator._service.patch(getId(beforeRecord), shallowClone(data), params).catch(() => {
      this._mutateStore('updated', beforeRecord, 2);
    });

    return Promise.resolve(afterRecord).then(select(params, ...this._alwaysSelect));
  }

  patch(uuid, data, params) {
    if (uuid === null) {
      return this._find(params).then(page => {
        return Promise.all(page.data.map(current => this._patch(current.uuid, data, params)));
      });
    }
  ...
  _remove(uuid, params) {
    ...
    // Start actual mutation on remote service
    this._replicator._service.remove(getId(beforeRecord), params).catch(() => {
      this._mutateStore('created', beforeRecord, 2);
    });

    return Promise.resolve(beforeRecord).then(select(params, ...this._alwaysSelect));
  }

  remove(uuid, params) {
    if (uuid === null) {
      return this._find(params).then(page => Promise.all(page.data.map(current => this._remove(current.uuid, params))));
    }
